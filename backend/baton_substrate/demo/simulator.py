...
class DemoSimulator:
    ...
    async def run(self) -> str:
        ...
        async with get_db() as db:
            mission = await mission_service.create_mission(
                db, "Substrate-Native Agent Coordination",
            ...
            for et in ENTITY_TYPES:
                await world_service.register_entity_type(
                    db, self.mission_id,
                    et["type_name"], et["json_schema"], et["invariants"],
                )

            for agent in AGENTS:
                await agent_service.register(
                    db, self.mission_id, agent.actor_id, agent.display_name, agent.role,
                )
    ...
    async def _step_1_research(self) -> None:
        ...
        async with get_db() as db:
            ...
            await causal_service.add_edge(
                db, self.mission_id, "atlas", "ev-001", "ev-002", "supports",
            )
    ...
    async def _step_2_planning(self) -> None:
        ...
        async with get_db() as db:
            ...
            await causal_service.add_edge(
                db, self.mission_id, "meridian", "ev-001", "ps-001", "derived_from",
            )
            await causal_service.add_edge(
                db, self.mission_id, "meridian", "ev-002", "ps-002", "derived_from",
            )
    ...
    async def _step_3_contention(self) -> None:
        ...
        async with get_db() as db:
            ...
            patch = Patch(ops=[
                PatchOp(op="upsert", type="Evidence", id="ev-003", value={
    ...
    async def _step_4_critique(self) -> None:
        ...
        async with get_db() as db:
            ...
            patch = Patch(ops=[
                PatchOp(op="upsert", type="Evidence", id="ev-004", value={
            ...
            await causal_service.add_edge(
                db, self.mission_id, "sentinel", "ev-004", "ev-001", "contradicts",
            )

            # Sentinel tries a patch with hard invariant violation (missing required field)
            bad_patch = Patch(ops=[
                PatchOp(op="upsert", type="PlanStep", id="ps-bad", value={
    ...
    async def _step_5_implementation(self) -> None:
        ...
        async with get_db() as db:
            ...
            patch = Patch(ops=[
                PatchOp(op="upsert", type="CodeArtifact", id="code-001", value={
                ...
                PatchOp(op="upsert", type="CodeArtifact", id="code-002", value={
            ...
            await causal_service.add_edge(
                db, self.mission_id, "forge", "ps-001", "code-001", "implemented_by",
            )
            await causal_service.add_edge(
                db, self.mission_id, "forge", "ps-002", "code-002", "implemented_by",
            )

            # Mark plan step as done
            done_patch = Patch(ops=[
                PatchOp(op="upsert", type="PlanStep", id="ps-001", value={
                    "title": "Design shared world model schema",
                    "description": "Define entity types and invariants for the coordination substrate",
                    "status": "done",
        ...
        async with get_db() as db:
            ...
            if result.accepted:
                ...
                await causal_service.invalidate_downstream(
                    db, self.mission_id, "ev-001", "atlas",
                )
    ...
    async def _step_7_energy_depletion(self) -> None:
        ...
        async with get_db() as db:
            ...
            # Large code artifact
            patch = Patch(ops=[
                PatchOp(op="upsert", type="CodeArtifact", id="code-003", value={
            ...
            try:
                await energy_service.spend(
                    db, self.mission_id, "forge", 150, "large_implementation_effort",
                )
            except ValueError:
                pass
            ...
            await baton_service.claim(db, self.mission_id, "meridian")
            decision_patch = Patch(ops=[
                PatchOp(op="upsert", type="Decision", id="dec-001", value={
            ...
            dec_result = await patch_service.propose(
                db, self.mission_id, "meridian", decision_patch,
            )
            if dec_result.accepted:
                await patch_service.commit(
                    db, self.mission_id, "meridian", dec_result.proposal_id,
                )

            # Link decision to evidence
            await causal_service.add_edge(
                db, self.mission_id, "meridian", "ev-001", "dec-001", "supports",
            )
            await causal_service.add_edge(
                db, self.mission_id, "meridian", "ev-003", "dec-001", "supports",
            )
